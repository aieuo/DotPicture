...
class dotpicture extends PluginBase implements Listener{
    public function onEnable(){
        	...
        	$this->config = new Config($this->getDataFolder() . "config.yml", Config::YAML, [
        		"wand_id" => 294,
        		"サーバー起動時にcreatepngフォルダ内のファイルをすべてpngに変換する" => false,
        		"png形式に変換したとき元画像を削除する" => false,
        		"FileSelect_FormId" => 543210,
        		"TypeSelect_FormId" => 543211,
        		"ChangeName_formId" => 543212,
        		"ConfirmDel_formId" => 543213,
        		"ChangeSize_formId" => 543214,
        		"CreateWall_formId" => 543215
        	]);
        	$this->config->save();
            if(!file_exists($this->getDataFolder()."createpng")){
			    @mkdir($this->getDataFolder()."createpng", 0721,true);
			    @mkdir($this->getDataFolder()."photos", 0721,true);
			    @mkdir($this->getDataFolder()."new_photos", 0721,true);
			}
        	$this->wandid = $this->config->get("wand_id");
        	$this->fileselect = $this->config->get("FileSelect_FormId");
        	$this->typeselect = $this->config->get("TypeSelect_FormId");
        	$this->changename = $this->config->get("ChangeName_formId");
        	$this->confirmdel = $this->config->get("ConfirmDel_formId");
        	$this->changesize = $this->config->get("ChangeSize_formId");
        	$this->createwall = $this->config->get("CreateWall_formId");

        	$this->del = $this->config->get("png形式に変換したとき元画像を削除する");

        	$createpng = $this->config->get("サーバー起動時にcreatepngフォルダ内のファイルをすべてpngに変換する");
			$dir = $this->getDataFolder()."createpng/";
        	if($createpng == true){
				$this->getLogger()->info("---------------変換しています---------------");
				if( is_dir($dir) && $handle = opendir($dir) ) {
					while( ($file = readdir($handle)) !== false ) {
						if( filetype( $path = $dir."".$file ) == "file" ) {
							$ext = substr($file,strrpos($file,'.') + 1);
							if($ext != "png"){
								$type = getimagesize($dir."".$file)[2];
								if($type == 1){
									$image = imagecreatefromgif($dir."".$file);
									$filename = str_replace(".".$ext,"",$file);
									imagepng($image,$dir."".$filename.".png");
									imagedestroy($image);
									$this->getLogger()->info($filename.".png を作成しました");
									if($this->del == true){
										unlink($dir.$file);
									}
								}elseif($type == 2){
				...
				$this->getLogger()->info("------------------完了しました------------------");
        	}
    }

    public function onBreak(BlockBreakEvent $event){
		$player = $event->getPlayer();
		if($player->getInventory()->getItemInHand()->getID() == $this->wandid and $player->isOp()){
			...
			switch ($args[0]) {
				...
				case 'png':
					...
					if(!strpos($args[1],".")!==false){
						$sender->sendMessage(".の後ろも入力してください");
						return true;
					}
					if (!file_exists($dir."".$args[1])){
						$sender->sendMessage("そんなファイルはありません");
						return true;
					}
					$type = getimagesize($dir."".$args[1])[2];
					if($type == 1){
						$image = imagecreatefromgif($dir."".$args[1]);
					}elseif($type == 2){
						$image = imagecreatefromjpeg($dir."".$args[1]);
					}else{
						$sender->sendMessage("その形式には対応していません");
						return true;
					}
					$ext = substr($args[1], strrpos($args[1], '.') + 1);
					$filename = str_replace(".".$ext,"",$args[1]);
					imagepng($image,$dir."".$filename.".png");
					imagedestroy($image);
					$sender->sendMessage($filename.".pngを作成しました");
					if($this->del == true){
						unlink($dir.$file);
					}
					return true;
